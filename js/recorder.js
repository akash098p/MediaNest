...
class RecorderManager {
  ...
  async startSystemAudioRecording() {
    try {
      ...
      if (!audioTracks.length) {
        ...
        throw new Error(
          "No system audio track was shared. Enable audio when selecting the screen/window.",
        );
      }
  ...
  startMediaRecording(stream, kind) {
    ...
    this.recording = true;
    window.App?.notify?.(
      kind === "video" ? "Screen recording started" : "Audio recording started",
    );

    const videoTrack = stream.getVideoTracks()[0];
    if (videoTrack)
      videoTrack.addEventListener("ended", () => this.stopRecording(), {
        once: true,
      });
  }
  ...
  finishRecording() {
    const mime =
      ...
      (this.recordingKind === "video" ? "video/webm" : "audio/webm");
