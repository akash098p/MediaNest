...
class TimelineManager {
  ...
  bindTimelineControls() {
    ...
    ruler?.addEventListener("click", (event) => {
      if (event.target.closest(".frame")) {
        ...
        const time = Math.max(
          0,
          (event.clientX - rect.left + ruler.scrollLeft) / this.pixelsPerSecond,
        );
    ...
    document
      .querySelectorAll("#tracksContainer .track-content")
      .forEach((content) => {
        content.addEventListener("click", (event) => {
          ...
          const time = Math.max(
            0,
            (event.clientX - rect.left + content.scrollLeft) /
    ...
    document
      .querySelectorAll("#tracksContainer .track-header button")
      .forEach((button) => {
        button.addEventListener("click", (event) => {
          event.stopPropagation();
          const row = button.closest(".track-row");
          const trackIndex = [
            ...document.querySelectorAll("#tracksContainer .track-row"),
          ].indexOf(row);
          ...
          const stateKey = action.includes("track-mute")
            ? "muted"
            : action.includes("track-lock")
              ? "locked"
              : action.includes("track-hide")
                ? "hidden"
                : "solo";
  ...
  async addClip(
    ...
    clip.startTime = preservePosition
      ? Math.max(0, Number(clip.startTime) || 0)
      : this.findFreeStart(track, clip.duration);
  ...
  renderClip(clip, track, trackIndex) {
    ...
    clipEl.querySelector(".clip-duration").textContent = this.formatTime(
      clip.duration,
    );
    clipEl.addEventListener("click", (e) => {
      if (!e.defaultPrevented) this.selectClip(clip.id);
    });
    ...
    clipEl.addEventListener("mousedown", (e) => {
      if (!e.target.closest(".clip-trim-handle"))
        this.onClipMouseDown(e, clip, trackIndex);
    });
    clipEl
      .querySelector(".clip-trim-left")
      .addEventListener("mousedown", (e) =>
        this.onTrimMouseDown(e, clip, trackIndex, "left"),
      );
    clipEl
      .querySelector(".clip-trim-right")
      .addEventListener("mousedown", (e) =>
        this.onTrimMouseDown(e, clip, trackIndex, "right"),
      );
  ...
  selectClip(clipId) {
    document
      ...
      .forEach((el) => el.classList.toggle("selected", el.id === clipId));
  ...
  onClipMouseDown(e, clip, trackIndex) {
    ...
    const onUp = () => {
      ...
      if (
        Math.abs(
          before[trackIndex].clips.find((c) => c.id === clip.id)?.startTime -
            clip.startTime,
        ) > 0.001
      )
        window.EditorManager?.addToHistory?.("move", before, after);
    };
  ...
  onTrimMouseDown(e, clip, trackIndex, side) {
    if (
      this.tracks[trackIndex]?.locked ||
      e.currentTarget.closest(".track-row")?.classList.contains("track-locked")
    ) {
      window.App?.notify?.("Track is locked");
      return;
    }
    ...
    const onMove = (ev) => {
      ...
      if (side === "left") {
        const newStart = Math.max(
          0,
          Math.min(originalEnd - minDuration, originalStart + delta),
        );
  ...
  normalizeTrack(trackIndex) {
    ...
    track.clips.forEach((clip) => {
      ...
      if (el) this.updateClipElement(el, clip);
    });
  }
  ...
  updateTimelineWidth() {
    ...
    const maxEnd = Math.max(
      10,
      ...this.getClips().map((c) => c.endTime || c.startTime + c.duration || 0),
    );
  ...
  getClips(trackIndex = null) {
    return trackIndex === null
      ? this.tracks.flatMap((t) => t.clips)
      : this.tracks[trackIndex]?.clips || [];
  }
  getFirstClip() {
    return this.getClips().sort((a, b) => a.startTime - b.startTime)[0] || null;
  }
  getNextClip() {
    const selected = document.querySelector(".timeline-clip.selected");
    return selected
      ? this.getClips().find((c) => c.id === selected.id) || this.getFirstClip()
      : this.getFirstClip();
  }
  displayClips(clips) {
    this.clearClips();
    return Promise.all(
      clips.map((clip) =>
        this.addClip(
          { ...clip },
          Number.isInteger(clip.trackIndex) ? clip.trackIndex : 0,
          true,
        ),
      ),
    );
  ...
  clearClips() {
    this.tracks.forEach((t) => {
      t.clips = [];
      if (t.element) t.element.innerHTML = "";
    });
  }
  snapshotTracks() {
    return JSON.parse(
  ...
  selectTrack(index) {
    if (this.tracks[index]) this.currentTrackIndex = index;
  }
  ...
  splitSelected() {
    ...
    const splitAt = Math.max(
      clip.startTime + 0.05,
      Math.min(at, clip.endTime - 0.05),
    );
    const firstDuration = splitAt - clip.startTime;
    const second = {
      ...clip,
      id: `clip-${Date.now()}-${this.clipCounter++}`,
      startTime: splitAt,
