...
class InspectorManager {
    ...
    render(){
        ...
        body.innerHTML=`<div class="si-title">${this.escape(clip.name||'Untitled')}</div>${audio?`<section><h4>Audio</h4>${this.range('Volume','volume',(clip.volume??1)*100,0,100,1,'%')}${this.range('Gain','gain',clip.gain??1,0,2,.01,'x')}${this.range('Pan','pan',clip.pan??0,-1,1,.01,'')}<label class="si-check"><input data-key="muted" type="checkbox" ${clip.muted?'checked':''}> Mute</label>${this.range('Fade In','fadeIn',clip.fadeIn??0,0,10,.1,'s')}${this.range('Fade Out','fadeOut',clip.fadeOut??0,0,10,.1,'s')}</section>`:''}<section><h4>Transform</h4>${this.range('Opacity','opacity',(clip.opacity??1)*100,0,100,1,'%')}${this.range('Scale','scale',(clip.scale??1)*100,10,300,1,'%')}${this.range('Rotation','rotation',clip.rotation??0,-180,180,1,'°')}${this.range('Position X','positionX',clip.positionX??0,-100,100,1,'%')}${this.range('Position Y','positionY',clip.positionY??0,-100,100,1,'%')}<label class="si-check"><input data-key="mirrorX" type="checkbox" ${clip.mirrorX?'checked':''}> Mirror horizontally</label><label class="si-check"><input data-key="mirrorY" type="checkbox" ${clip.mirrorY?'checked':''}> Mirror vertically</label></section><section><h4>Canvas</h4><label class="si-select">Aspect ratio <select data-key="aspectRatio"><option value="16 / 9" ${(clip.aspectRatio??'16 / 9')==='16 / 9'?'selected':''}>16:9 YouTube</option><option value="9 / 16" ${clip.aspectRatio==='9 / 16'?'selected':''}>9:16 TikTok / Reels</option><option value="1 / 1" ${clip.aspectRatio==='1 / 1'?'selected':''}>1:1 Square</option><option value="4 / 5" ${clip.aspectRatio==='4 / 5'?'selected':''}>4:5 Social</option></select></label></section><section><h4>Playback</h4>${this.range('Speed','playbackRate',clip.playbackRate??1,.25,4,.05,'x')}</section>`;
        body.querySelectorAll('input[data-key], select[data-key]').forEach(input=>input.addEventListener('input',()=>{let key=input.dataset.key;let value=input.type==='checkbox'?input.checked:input.tagName==='SELECT'?input.value:Number(input.value);if(key==='volume')value/=100;if(key==='opacity'||key==='scale')value/=100;this.set(clip,key,value);const out=input.parentElement.querySelector('output');if(out)out.textContent=`${Number(input.value).toFixed(2)}${input.dataset.key==='volume'||input.dataset.key==='opacity'||input.dataset.key==='scale'?'%':''}`;this.applyTransform(clip);}));
        this.applyTransform(clip);
    }

    applyTransform(clip){const video=document.getElementById('videoPreview');const preview=document.getElementById('previewContainer');if(!video||clip.type!=='video')return;video.style.opacity=clip.opacity??1;video.style.transform=`translate(${clip.positionX??0}%, ${clip.positionY??0}%) scale(${(clip.mirrorX?-1:1)*(clip.scale??1)}, ${(clip.mirrorY?-1:1)*(clip.scale??1)}) rotate(${clip.rotation??0}deg)`;if(clip.aspectRatio&&preview)preview.style.aspectRatio=clip.aspectRatio;}
