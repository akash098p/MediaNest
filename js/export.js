...
class ExportManager {
  ...
  setFormat(format) {
    this.exportFormat = String(format || "WEBM").toUpperCase();
    this.currentFormat = this.exportFormat;
  }
  setQuality(quality) {
    this.exportQuality = quality || "High";
  }
  ...
  getMimeType() {
    ...
    const preferred =
      formats[this.exportFormat] || "video/webm;codecs=vp8,opus";
  ...
  async exportProject() {
    ...
    if (this.exportFormat === "MP4" && mime !== "video/mp4") {
      this.notify(
        "MP4 export is not supported by this browser. Exporting WebM instead.",
      );
    }
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        recorder.onerror = () =>
          reject(recorder.error || new Error("Export failed"));
        recorder.start(250);
