...
class EffectsManager {
  ...
  bindControls() {
    ...
    bindings.forEach(([id, method]) =>
      document
        .getElementById(id)
        ?.addEventListener("input", (event) =>
          this[method](Number(event.target.value)),
        ),
    );
    document
      .querySelectorAll('[id^="eqBand"]')
      .forEach((input) =>
        input.addEventListener("input", (event) =>
          this.setEqualizerBand(
            Number(input.id.replace("eqBand", "")),
    ...
    document
      ...
      ?.addEventListener("click", () => this.normalizeVolume());
    document
      .getElementById("fadeInSeconds")
      ?.addEventListener("input", (event) =>
        this.setFadeIn(Number(event.target.value)),
      );
    document
      .getElementById("fadeOutSeconds")
      ?.addEventListener("input", (event) =>
        this.setFadeOut(Number(event.target.value)),
      );
    document
  ...
  setNoiseReduction(amount) {
    ...
    const clip = this.selectedClip();
    if (clip) clip.noiseReduction = amount;
  ...
  setFadeIn(seconds) {
    ...
    const clip = this.selectedClip();
    if (clip) window.PlayerManager?.setFadeIn?.(seconds, clip);
  ...
  setFadeOut(seconds) {
    ...
    const clip = this.selectedClip();
    if (clip) window.PlayerManager?.setFadeOut?.(seconds, clip);
  ...
  setPitchShift(semitones) {
    ...
    const clip = this.selectedClip();
    if (clip) clip.pitchShift = semitones;
  ...
  setEqualizerBand(index, value) {
    if (index >= 0 && index < this.equalizerBands.length) {
      ...
      const clip = this.selectedClip();
      if (clip) clip.equalizerBands = [...this.equalizerBands];
      window.PlayerManager?.applyEffects?.();
  ...
  setSpeed(value) {
    ...
    if (clip) {
      ...
      window.PlayerManager?.setPlaybackRate?.(value);
    }
